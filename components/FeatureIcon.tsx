...
const FeatureIcon: React.FC<FeatureIconProps> = ({ icon, size = 'md', animated = true }) => {
  ...
  const WrapperContent = (
    <>
      <Box
        ...
        inset={0}
        borderRadius="xl"
        className="blur group-hover:blur-lg transition-all bg-gradient-to-br from-primary-500/20 to-secondary-500/20"
      />
  ...
  return animated ? (
    <motion.div
      style={{
        width: `${sizeMap[size] * 4}px`,
        height: `${sizeMap[size] * 4}px`,
        position: 'relative',
      }}
      className="group"
